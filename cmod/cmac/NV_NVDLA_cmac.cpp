...
void NV_NVDLA_cmac::sc2mac_dat_b_transport(int ID, nvdla_sc2mac_data_if_t* payload, sc_time& delay) {
    ...
    if ( 1 == payload->pd.nvdla_stripe_info.stripe_st ) {
        ...
        UpdateWeightFromShadowToActive();             //一个类里面的，可以直接调用。在类外面，需要加类名
...
//上面都是对NV_NVDLA_cmac类中函数的定义
//定义完后，这里生成了一个实体
NV_NVDLA_cmac * NV_NVDLA_cmacCon(sc_module_name name)   //粗看以为在做乘法，细看才发现是声明的一个类指针
{
