...
};  //这个就是NV_NVDLA_cmac的类声明。相关的函数定义在NV_NVDLA_cmac.cpp中。下面的extern “C"，应该就是把这些代码与这个.h文件关联？
//据查，这个NV_NVDLA_cmac类（CMAC部分的顶层类），只在/cmod/nvdla_core/NV_NVDLA_core.h文件中定义了两个实际对象。
//所以可以看出，这些函数在nvdla_core中，做了实际调用。

SCSIM_NAMESPACE_END()   //这是啥？

extern "C" scsim::cmod::NV_NVDLA_cmac * NV_NVDLA_cmacCon(sc_module_name module_name);
//extern，还记得以前写C，调用另一个文件中的数据，或者是调用另一个文件中函数。都需要先在当前文件进行extern。感觉基本是一样的
